...
export default function AdminLogs(){
  ...
  useEffect(()=>{
    fetch("/api/admin/logs",{cache:"no-store"}).then(async response=>{
  ...
  return <main className="mx-auto max-w-7xl px-6 py-14">
    ...
    <section className="mt-8 rounded-3xl border border-white/10 bg-zinc-950 p-6">
      {logs.length?<div className="space-y-3">{logs.map(log=><article key={log.id} className="rounded-2xl border border-white/10 p-4"><div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between"><p className="font-bold text-orange-300">{log.action}</p><p className="text-xs text-zinc-600">{new Date(log.createdAt).toLocaleString("id-ID")}</p></div><p className="mt-1 text-xs text-zinc-500">{log.entityType||"—"} · {log.entityId||"—"} · member {log.memberId||"system"}</p>{log.metadata&&<pre className="mt-3 overflow-auto rounded-xl bg-black p-3 text-xs text-zinc-500">{JSON.stringify(log.metadata,null,2)}</pre>}</article>)}</div>:<p className="text-zinc-500">Belum ada activity log.</p>}
    </section>
  </main>;
}