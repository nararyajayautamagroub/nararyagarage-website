...
export async function GET(_req:Request,{params}:{params:Promise<{id:string}>}){
  ...
  try{
    const data=await prisma.forumPost.findMany({where:{threadId:id},orderBy:{createdAt:"asc"},take:200});
...
export async function POST(req:Request,{params}:{params:Promise<{id:string}>}){
  ...
  try{
    ...
    const post=await prisma.forumPost.create({data:{threadId:id,authorId:session.user.id,body:body.body},select:{id:true,createdAt:true}});
