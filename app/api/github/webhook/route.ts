...
export async function POST(req:Request){
  ...
  revalidatePath("/api/github/repositories");
  revalidatePath("/api/github/activity");
  if(repository){
    const [owner,name]=repository.split("/");
    if(owner&&name){
      revalidatePath("/repositories/"+encodeURIComponent(owner)+"/"+encodeURIComponent(name));
      revalidatePath("/api/github/repository/"+encodeURIComponent(owner)+"/"+encodeURIComponent(name));
    }
  }
