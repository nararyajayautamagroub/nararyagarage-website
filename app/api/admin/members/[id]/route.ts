...
const schema=z.object({
  ...
  userRole:z.string().trim().min(2).max(30).optional()
});
...
export async function PATCH(req:Request,{params}:{params:Promise<{id:string}>}){
  ...
  try{
    ...
    const member=await prisma.member.findUnique({where:{id},select:{id:true,userId:true}});
    ...
    const updated=await prisma.$transaction(async tx=>{
      ...
      if(body.userRole!==undefined){
        await tx.user.update({where:{id:member.userId},data:{role:body.userRole.toUpperCase()}});
      }
      return result;
    });
