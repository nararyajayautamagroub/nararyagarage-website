...
export async function GET(){
  ...
  const data=await prisma.notification.findMany({where:{memberId:member.id},orderBy:{createdAt:"desc"},take:100,select:{id:true,title:true,body:true,channel:true,readAt:true,createdAt:true}});
...
export async function PATCH(req:Request){
  ...
  try{
    ...
    const updated=await prisma.notification.update({where:{id:notification.id},data:{readAt:body.read?new Date():null},select:{id:true,readAt:true}});
